let isSnowing=true;
let personName="sara";
let personAge=21;

let sarahObject={
    name: personName,
    age:personAge,
    isSnowing:isSnowing
}

//our condition if snows she cannot come to school

if(sarahObject.isSnowing==true) {
    console.log("sara cannot come to school");
}else {
    console.log("sara can come to school");
}

//if she's under 18 she cannot drive a car

if (sarahObject.age<18) {
    console.log("sara cannot drive a car");
}else {
    console.log("sara can drive a car");
}

adminCredentials= {
    username: "admin",
    password:"1234",
}

if (adminCredentials.username=="admin" && adminCredentials.password=="1234"){
    console.log("Welcome Admin");
}else {
    console.log("Username or password incorrect");
}

// else if
// if a person is under 18, no tax, if a person between 18 and 30 tax rate 10%
// if a person over 30 tax rate is 30% and all other conditions 50%

let joeObject = {
    name:"Joe",
    age: 60,
    income:1000000
}

class Person {
    constructor(name,age,income,taxRate){
        this.name=name;
        this.age=age;
        this.income=income;
        this.taxRate=taxRate;
    }
    showNetto() {
        return this.income*(1-this.taxRate);
    }
}

//create an instance
let person = new Person("Henry",31,10000,0.3);
console.log(person.showNetto());

let taxRate=0;

//let console.log the starting init tax rate value
console.log(`the person's tax rate is now ${taxRate}`);

if (joeObject.age<18) {
    taxRate=0;
}else if (joeObject.age>=18 && joeObject.age<30){
    taxRate=0.1;
}else if (joeObject.age>=30 && joeObject.age<=50){
    taxRate=0.3;
}else if (joeObject.age>50){
    taxRate=0.5;
}

//end of if block

//let console.log the starting init tax rate value
let nettoIncome=joeObject.income*(1-taxRate);
console.log(`${joeObject.name}'s tax rate is now ${taxRate}\n${joeObject.name}'s net income is ${nettoIncome}$`);